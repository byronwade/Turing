...
Status: candidate evaluation and operating baseline; only the dependency-free M0 toolchain is active  
...
The contained M0 source foundation currently uses:

- Rust `1.97.1`, Cargo resolver 3, Rust 2024 for new crates;
- rustfmt and Clippy with warnings denied in CI;
- Python 3 standard library only for repository validators and deterministic generation;
- a committed Cargo lockfile;
- no external Rust runtime dependencies;
- no native dependencies, build scripts, FFI, or approved unsafe code.

The initial generated control-plane contract uses JSON as the reviewed source and a repository-owned Python generator. The generated Rust has no runtime JSON or Python dependency. `python3 -B tools/generate_ipc.py --check` verifies byte-for-byte output determinism.

This generator arrangement is an M0 implementation choice, not a commitment that production protocol generation will remain Python-based. Any replacement must preserve deterministic output, stable IDs, schema validation, source mappings, compatibility review, and a no-network generation path.
...
A code generator additionally requires:

- a canonical input schema and version;
- validation before code generation;
- stable identifier and compatibility rules;
- deterministic output checked in CI;
- no undeclared network access;
- a registered generated-code ledger entry;
- review of both source and generated diff;
- rollback and regeneration instructions.
