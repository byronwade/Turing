...
|---|---|---|---|
| Identity and launch | token, integrity level, AppContainer/capabilities, package identity, parent and child process IDs | uid/gid, user namespace, PID namespace, parent-death policy, capabilities, process IDs | code-signing identity, entitlements, sandbox profile, audit/process identity, XPC peer identity |
...
| Resource controls | job limits, handle allowlist, memory/CPU limits | cgroups, rlimits, namespaces, descriptor limits | role-specific limits, launch constraints, descriptor and helper limits |
