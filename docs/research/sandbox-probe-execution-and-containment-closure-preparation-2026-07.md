...
| [Windows Create Process In Sandbox APIs](https://learn.microsoft.com/en-us/windows/win32/secauthz/createprocessinsandbox) | The documented launch specification separates AppContainer mode from integrity, capability, filesystem, network, UI-restriction, and mitigation fields; several fields do not take effect when AppContainer is disabled. | Capture the effective launch mode and applied restriction state for the exact process. A requested field or a launch API description is not proof that the restriction was enforced. |
...
| [Linux user namespaces and resource control](https://docs.kernel.org/admin-guide/namespaces/resource-control.html) | Linux kernel guidance warns that user namespaces can expose systems to resource misuse and recommends memory control groups when user namespaces are enabled. | Capture namespace limits and cgroup/resource-control state alongside user-namespace identity; namespace creation alone is not bounded-resource or containment evidence. |
