...
The validator also requires every active question to have at least one `docs/research/` evidence route and resolves every `evidence_start` entry in the readiness crosswalk inside the repository. The current crosswalk routes all 37 active questions to research evidence; it has 10 lanes and 256 evidence-start entries, all of which resolve to an existing file or directory. This catches missing research coverage and stale route references during documentation changes while preserving no-claim status. It does not verify that the referenced evidence is sufficient, independently reviewed, or accepted for implementation.
