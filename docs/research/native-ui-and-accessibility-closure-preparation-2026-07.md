...
The official [Windows Narrator guide](https://support.microsoft.com/en-us/accessibility/windows/narrator/complete-guide-to-narrator), [Apple VoiceOver User Guide](https://support.apple.com/guide/voiceover/welcome/mac), and [GNOME Orca guide](https://help.gnome.org/orca/) make the manual interaction model a separate evidence input. They cover navigation modes, keyboard or braille interaction, structural navigation, forms, live regions, profiles, and configuration. A future review must record the exact assistive-technology version/configuration, navigation mode, transcript or action record, focus and announcement timing, and unsupported workflows; a platform API tree alone is not a screen-reader workflow result.

The checked [accessibility-source manifest](../accessibility/machine/accessibility-source-manifest.json), validated by [`validate_accessibility_sources.py`](../../tools/validate_accessibility_sources.py), records these platform and assistive-technology source identities and evidence consequences. It is a no-claim source record only; it does not provide accessibility workflow execution, screen-reader coverage, IME correctness, page-tree proof, or UI-gate approval.
