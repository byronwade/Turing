...
The checked [owner-decision synchronization matrix](../project-buildout/machine/owner-decision-synchronization.json), [schema](../project-buildout/machine/owner-decision-synchronization.schema.json), and [validator](../../tools/validate_owner_decision_synchronization.py) now define all 11 canonical closure scopes used by the PB-020 review template. Each scope identifies the owner role, independent reviewer role, minimum evidence classes, exact synchronization paths, the canonical task queue, affected proposed task manifests, and the active `TASK-000011` manifest where applicable, exception policy, and claims prohibited until review.
