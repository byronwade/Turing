...
The dated [Sandbox Platform-Evidence Decision Preparation](sandbox-platform-evidence-decision-prep-2026-07.md) records the platform-specific evidence differences for Windows, Linux, and macOS, including the rule that seccomp is not a complete sandbox, that Landlock ABI support must be captured explicitly, that Windows sandbox restriction fields depend on the effective AppContainer mode, and that Linux user namespaces require resource-control scrutiny. The checked [sandbox platform-source manifest](../security-engine/machine/sandbox-platform-source-manifest.json) and [`validate_sandbox_platform_sources.py`](../../tools/validate_sandbox_platform_sources.py) keep eight official source records and nine evidence axes synchronized without turning documentation into containment evidence.
