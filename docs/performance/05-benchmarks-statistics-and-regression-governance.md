...
- The checked [Benchmark engine baseline harness readiness map](../research/benchmark-engine-baseline-harness-readiness-map-2026-07.md), which organizes current no-claim `PB-013` evidence into Level 0 through Level 3 stop/resume guidance without providing a browser run, raw sample, owner-reviewed benchmark readiness, competitor result, Chrome-class evidence, or performance claim.
- Claim-bundle registry references that name the statistics-analysis plan used for public-claim interpretation. [`tools/validate_benchmark_claim_bundles.py`](../../tools/validate_benchmark_claim_bundles.py) cross-checks that `statistics_analysis_plan_id` against the checked plan so raw samples cannot bypass the analysis contract.
- Benchmark readiness-review scopes that name the statistics-analysis plan accepted for readiness. [`tools/validate_benchmark_readiness_review.py`](../../tools/validate_benchmark_readiness_review.py) keeps `review_scope.statistics_analysis_plan` null in the checked no-claim template and requires future evidence beyond that template before review can support claims.
