...
The checked no-claim [Reference Platform Support Scorecard](../research/reference-platform-support-scorecard-2026-07.md) and [`reference-platform-scorecard.json`](machine/reference-platform-scorecard.json) make `PB-006` candidate scope, cross-platform evidence dimensions, source observations, open questions, and unsupported rows explicit. They do not select Windows, macOS, or Linux, establish support, or replace clean-host, native-shell, accessibility, sandbox, benchmark, packaging, incident, or owner-review evidence.
