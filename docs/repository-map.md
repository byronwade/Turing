...
- `validate_fresh_host_toolchain_sources.py`: dependency-free validation for the checked no-claim PB-008/PB-009 Rustup, Cargo, and Microsoft toolchain-source manifest, nine toolchain/fresh-host evidence axes, source-document links, and unsupported reproduction/readiness boundaries; it does not prove compiler/SDK/linker equivalence, independent reproduction, or implementation;
...
The checked no-claim [`fresh-host-toolchain-source-manifest.json`](project-buildout/machine/fresh-host-toolchain-source-manifest.json) and [`fresh-host-toolchain-source-manifest.schema.json`](project-buildout/machine/fresh-host-toolchain-source-manifest.schema.json) track official Rustup, Cargo, and Microsoft toolchain observations; [`validate_fresh_host_toolchain_sources.py`](../tools/validate_fresh_host_toolchain_sources.py) verifies the records without proving an independent run, toolchain equivalence, owner review, or readiness promotion.
