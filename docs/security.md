...
Security design and review use [Chromium as a security reference](security-engine/reference-chromium-security.md) — the [`chromium/chromium`](https://github.com/chromium/chromium) codebase and its security-process material are the primary reference for threat models, exploit classes, mitigations, and severity calibration. It is a learning reference only: `AGENTS.md` forbids any release-path dependency on Chromium/Blink/V8, and "Chromium does X" is never sufficient justification without an independent rationale.
