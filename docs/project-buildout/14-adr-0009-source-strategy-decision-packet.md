...
Result: `ADR9-EV-007` remains partial. The probe provides clean-target failure and partial generated-output evidence, but Turing still needs an owner-selected baseline and feature profile, feature-correct full clean-target regeneration, independent-host comparison, generator manifests, source-to-output license/provenance mapping, accepted media and MozJS feature policy, and dynamic build-script/proc-macro tracing before generated outputs can be trusted.
