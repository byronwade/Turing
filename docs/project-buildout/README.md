...
13. [Implementation Master Plan](implementation-plan/README.md)
...
The [Implementation Master Plan](implementation-plan/README.md) is the dependency-ordered execution guide for agents and maintainers. It defines M0 through M9, the WP-001 through WP-018 critical path, decision gates, interface freeze points, evidence classes, agent start/stop rules, work-package playbooks, handoffs, and stable-release sequence.

The plan does not authorize a monolithic autonomous build. A source task still requires a reviewed `TASK-*` manifest and accepted dependencies.
