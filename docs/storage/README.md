...
The checked no-claim [profile/session source manifest](machine/profile-session-source-manifest.json), [manifest schema](machine/profile-session-source-manifest.schema.json), and [`validate_profile_session_sources.py`](../../tools/validate_profile_session_sources.py) track official WHATWG/W3C web-storage, transaction, clearing, and privacy observations across nine axes. They do not define the Turing profile format, migration protocol, credential boundary, sync behavior, durability guarantee, or data-loss readiness.
