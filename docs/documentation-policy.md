...
The research log is a handoff surface. Keep dated entries in newest-first order and keep the reusable entry template after all dated entries so readers do not have to search past older material for newer evidence. The research index must keep the current implementation-research lane map with a `Must not claim` column and the build-readiness research crosswalk with primary `RQ-*`, `PB-*`, and `TASK-*` links so continuation guidance stays tied to unsupported-claim boundaries. The machine companion `blueprint-v1/machine/research-readiness-crosswalk.json` must stay synchronized with that prose and must not promote readiness or task authority. Active research packets must visibly distinguish source observations from candidate inferences or recommendations, unresolved next work, and dated stable source context; `tools/validate_research_index.py` enforces that minimum quality boundary. The documentation-readiness matrix in `project-buildout/18-documentation-readiness-evidence-matrix.md` must keep the current `PB-001` evidence, objective-to-evidence mapping, continuation checks, drift triggers, validation commands, and no-claim boundary synchronized with the entry points and machine registries. The checked documentation-readiness completion audit in `project-buildout/machine/documentation-readiness-completion-audit.json` must stay synchronized with the matrix, entry points, `PB-020`, and unsupported all-information-ready-for-building boundary. The checked build-information readiness ledger in `project-buildout/machine/build-information-readiness-ledger.json` must stay synchronized with the matrix, research index, `PB-020`, and source-strategy/fresh-host/IPC/sandbox/benchmark/native-shell/profile-session/package-update/incident-response/backup-ownership/task-authority/Chrome-class product information gaps before any all-information-ready-for-building language changes. The checked `project-buildout/19-github-issue-handoff.md` document and `project-buildout/machine/github-issue-handoff.json` snapshot must stay synchronized after issue or branch cleanup and must remain a coordination handoff only, not task approval, readiness promotion, implementation proof, or live GitHub verification. Documentation-readiness and handoff changes must satisfy the corresponding [Definition of Done](blueprint-v1/20-definition-of-done.md) criteria. `start-here.md`, `docs/README.md`, and the root `README.md` must keep the current stop/resume map so a new maintainer can find gate truth, the source-strategy/fresh-host/IPC/sandbox/benchmark/native-shell/profile-session/package-update/incident-response/ownership lane set, task shaping, source-strategy evidence, benchmark evidence, and operating controls before implementation.
